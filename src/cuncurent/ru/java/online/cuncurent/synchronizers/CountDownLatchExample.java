...
/**
 * CountDownLatch («защелка с обратным отсчетом») — объект синхронизации потоков,
 * блокирующий один или несколько потоков до тех пор, пока не будут выполнены
 * определенные условия. Количество условий задается счетчиком.
 * При обнулении счетчика, т.е. при выполнении всех условий, блокировки
 * выполняемых потоков будут сняты и они продолжат выполнение кода.
 * Примером CountDownLatch может служить экскурсовод, собирающий группу из
 * заданного количества туристов. Как только группа собрана она отправляется на
 * экскурсию. Необходимо отметить, что счетчик одноразовый и не может быть
