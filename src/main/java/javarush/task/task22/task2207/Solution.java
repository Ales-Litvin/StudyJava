...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while ((data = br.readLine()) != null){
            stringSum.append(" ").append(data).append(" ");
            }
        br.close();
        ...
        for (String str : new String(stringSum).split(" ")){
            ...
            Pair pairRevers = new Pair();
            pairRevers.first = stringRevers;
            pairRevers.second = str;

            if (result.indexOf(pair) < 0 && result.indexOf(pairRevers) < 0) {
                if (stringSum.indexOf(" " + stringRevers + " ") >= 0) {
                    result.add(pair);
                    int firstIndex = stringSum.indexOf(" " + str + " ") + 1;
                    stringSum.delete(firstIndex, firstIndex + str.length());
                    int lastIndex = stringSum.indexOf(" " + stringRevers + " ") + 1;
                    stringSum.delete(lastIndex, lastIndex + stringRevers.length());
                }
            }
        }
